...
class Student:
    ...
    def __init__(self,root):
        ...
        title=Label(self.root,text="Teacher Management System",bd=10,relief=GROOVE,font=("calibri",30,"bold"),bg="black",fg="orange")
...
#==============Manage Frame====================
        ...
        #===========Title Label==========
        m_title=Label(Manage_Frame,text="Manage Teacher",bg="black",fg="orange",font=("calibri",30,"bold"))
        m_title.grid(row=0,columnspan=2,pady=20)

        #===========Roll No. Label==========
        lbl_roll=Label(Manage_Frame,text="Teacher Id",bg="black",fg="orange",font=("calibri",20,"bold"))
        lbl_roll.grid(row=1,column=0,pady=10,padx=20,sticky="w")

        #============Roll no. Entry Field =========
        txt_Roll=Entry(Manage_Frame,textvariable=self.id_var,font=("calibri",15,"bold"),bd=5,relief=GROOVE)
...
#==============Detail Frame====================   
        ...
        combo_search['values']=("Teach_Id","Name","Contact")
...
#==============Table Frame====================== 
        ...
        self.Teacher_table=ttk.Treeview(Table_Frame,columns=("teach id","name","email","gender","contact","dob","address"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.Teacher_table.heading("teach id",text="Teacher Id")
        ...
        self.Teacher_table.column("teach id",width=50)
...
#====================Validation on Required Fields======================
    def add_student(self):
        if self.id_var.get()=="" :
                ...
                cur=con.cursor()
                cur.execute("insert into teachers values(%s,%s,%s,%s,%s,%s,%s)",(self.id_var.get(),
...
#=============Create update_data function=========
    def update_data(self):
        con=pymysql.connect(host="localhost", user="root", password="", database = "stud")    
        cur=con.cursor()
        cur.execute("update teachers set name=%s, email=%s, gender=%s, contact=%s, dob=%s, address=%s  where teach_id=%s",
                                                                (self.name_var.get(),
...
#=============Create delete_data function=========
    def delete_data(self):
        con=pymysql.connect(host="localhost", user="root", password="", database="stud")    
        cur=con.cursor()
        cur.execute("delete from teachers where teach_id=%s",self.id_var.get())
        con.commit()
        con.close() 
...
#========Create search_data function==========
    def search_data(self):
        con=pymysql.connect(host="localhost", user="root", password="", database="stud")    
        cur=con.cursor()
        cur.execute("select * from teachers where "+str(self.search_by.get())+" LIKE '%"+str(self.search_tx.get())+"%'")
        rows = cur.fetchall()
